...
bounded_strips(Bound, Initial, Final, Visited, [Action|Actions]) :- succ(Predecessor, Bound), action(Initial, Action), perform(Initial, Action, Intermediate), \+ member(Intermediate, Visited),
bounded_strips(Predecessor, Intermediate, Final, [Intermediate|Visited], Actions).
action(State, move(Block, Destination)) :- block(Block), \+ Block == Destination,free(State, Block),free(State, Destination).
free(State, Thing) :- thing(Thing), \+ member(on(_, Thing), State).
thing(Block) :- block(Block).
thing(Place) :- place(Place).
perform(Source, move(Block, Destination), Target) :-substitute(on(Block, _), Source, on(Block, Destination), Target).
substitute(_, [], _, []).
